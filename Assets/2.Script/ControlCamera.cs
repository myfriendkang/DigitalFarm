...
public class ControlCamera : MonoBehaviour {
	...
	void Start () {
	    ...
	    targetPosition1 = new Vector3 (-111, 30, -236);
	...
	public void FocusObj(int objNum){
		if (objNum == 1) {
			...
			//Vector3 targetPosition = new Vector3 (-111, 30, -236);//targetObj1.TransformPoint (new Vector3 (0, 0, -10));
			//mainCamera.transform.position = Vector3.SmoothDamp (mainCamera.transform.position, targetPosition, ref velocity, smoothTime * Time.deltaTime);
		} else if (objNum == 2) {
			StopAllCoroutines ();
			StartCoroutine ("ChangeMove",targetPosition2);
			//Vector3 targetPosition2 = new Vector3 (1, 140, -236);//targetObj2.TransformPoint (new Vector3 (0, 0, -10));
			//mainCamera.transform.position = Vector3.SmoothDamp (mainCamera.transform.position, targetPosition2, ref velocity, smoothTime * Time.deltaTime);
		} else if (objNum == 3) {
			StopAllCoroutines ();
			StartCoroutine ("ChangeMove",targetPosition3);
			//Vector3 targetPosition3 = new Vector3 (-85.6f, 140, -236);//targetObj2.TransformPoint (new Vector3 (0, 0, -10));
			//mainCamera.transform.position = Vector3.SmoothDamp (mainCamera.transform.position, targetPosition2, ref velocity, smoothTime * Time.deltaTime);
		} else if (objNum == 4) {
			StopAllCoroutines ();
			StartCoroutine ("ChangeMove",targetPosition4);
			//Vector3 targetPosition4 = new Vector3 (-12.2f, 35.7f, -236);//targetObj2.TransformPoint (new Vector3 (0, 0, -10));
			//mainCamera.transform.position = Vector3.SmoothDamp (mainCamera.transform.position, targetPosition2, ref velocity, smoothTime * Time.deltaTime);
		} else if (objNum == 5) {
			StopAllCoroutines ();
			StartCoroutine ("ChangeMove",targetPosition5);
			//Vector3 targetPosition5 = new Vector3 (-128.4f, 89.7f, -236);//targetObj2.TransformPoint (new Vector3 (0, 0, -10));
			//mainCamera.transform.position = Vector3.SmoothDamp (mainCamera.transform.position, targetPosition2, ref velocity, smoothTime * Time.deltaTime);
		}
	}
